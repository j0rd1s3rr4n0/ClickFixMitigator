...
function markDetectionOnPage() {
  if (hasDetectionsOnPage) {
    return;
  }
  hasDetectionsOnPage = true;
...
const FULLSCREEN_BANNER_ID = "clickfix-mitigator-fullscreen-banner";
...
function showFullscreenNotice(messageText, { sticky } = {}) {
  ...
  if (existing) {
    ...
    existing.dataset.sticky = sticky ? "true" : "false";
    return;
  }
  const root = document.body || document.documentElement;
  if (!root) {
  ...
  const banner = document.createElement("div");
  banner.id = FULLSCREEN_BANNER_ID;
  banner.dataset.sticky = sticky ? "true" : "false";
  const primaryBanner = document.getElementById("clickfix-mitigator-banner");
  const offsetTop = primaryBanner ? Math.round(primaryBanner.getBoundingClientRect().bottom + 12) : 16;
  banner.style.cssText = [
    "position:fixed",
    `top:${offsetTop}px`,
    "right:16px",
  ...
  const bannerIconUrl = CLICKFIX_ICON_DATA_URL;
  if (bannerIconUrl) {
  ...
  const message = document.createElement("div");
  message.textContent = messageText || t("fullscreenExitPrompt");
  message.setAttribute("data-clickfix-fullscreen-message", "true");
  message.style.cssText = [
    "line-height:1.4",
    "font-size:12px",
    "color:#f8fafc"
...
function hideFullscreenNotice() {
  const existing = document.getElementById(FULLSCREEN_BANNER_ID);
  if (existing && existing.dataset.sticky !== "true") {
    existing.remove();
  }
}
...
function exitFullscreenDueToDetection() {
  if (!isTopFrame || !isFullscreenActive()) {
    return;
  }
  exitFullscreen();
  ensureLocaleReady().then(() => {
    showFullscreenNotice(t("fullscreenBlockedDetection"), { sticky: true });
  });
}
...
function startFullscreenMonitor() {
  if (!isTopFrame || fullscreenListenerAttached) {
    return;
  }
