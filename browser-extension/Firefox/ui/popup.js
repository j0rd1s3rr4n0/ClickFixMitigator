...
function renderQuickSummary(settings) {
  ...
  if (quickSummaryMode) {
    quickSummaryMode.textContent = isEnabled ? "Activo" : "Pausado";
  }
  if (quickSummaryLastAlert) {
    quickSummaryLastAlert.textContent = lastEntry
      ? `${lastEntry.hostname || "Sitio"}${lastScore !== null && lastScore !== undefined ? ` · ${lastScore}/100` : ""}`
      : "Sin alertas";
  }
...
async function initPopupOnboarding() {
  ...
  const skip = async () => {
    if (popupOnboardingLanguage) {
      await chrome.storage.local.set({ uiLanguage: normalizeLocale(popupOnboardingLanguage.value || browserLocale || "es") });
    }
