...
$manifestTemplatePath = Join-Path $sourceRoot "manifest.firefox.android.json"

if (-not (Test-Path $manifestTemplatePath)) {
  throw "Missing Firefox Android manifest template: $manifestTemplatePath"
}

$manifest = Get-Content $manifestTemplatePath -Raw | ConvertFrom-Json
$version = $manifest.version
if (-not $version) {
  throw "Firefox Android manifest template does not define a version."
}
...
$stageRoot = Join-Path $outputRootPath "firefox-android"
$packageName = "clickfix-mitigator-firefox-android-$version.xpi"
...
Write-Host "Firefox Android stage:" $stageRoot
Write-Host "Firefox Android package:" $packagePath
